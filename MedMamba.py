...
from TextAttention2 import TextAttention
import numpy as np 
from mil_head_v2 import LocalTypicalityMILHead
try:
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
        ...
        # text guide 
        self.ta1 = TextAttention(96, 3)
        self.ta2 = TextAttention(192,  96)
        self.ta3 = TextAttention(384, 192)
        self.ta4 = TextAttention(768, 384)
        
        self.text_layers = nn.ModuleList([self.ta1, self.ta2, self.ta3, self.ta4])
        ...
        # 替换原有的分类头为MIL分类头
        self.mil_head = LocalTypicalityMILHead(in_features=self.num_features, num_classes=num_classes)
    ...
    def forward_backbone(self, x, y):
        ...
        x = self.pos_drop(x)
        for layer, text_layer in zip(self.layers, self.text_layers):
            # print(text_layer)
            y = text_layer(x, y)
            temp = y 
    ...
    def forward(self, x, patients_text, labels=None):
        """
        Args:
            x: 输入图像 [B, C, H, W]
            patients_text: 患者特征 [B, 1, 3]
            labels: 包级别的标签 [B]
        """
        x = self.forward_backbone(x, patients_text)  # 使用患者特征进行特征提取
        
        # 重新组织特征以适应MIL
        B, H, W, C = x.shape
        x = x.view(B, H*W, C)  # 将空间维度展平作为实例 (B, N, C)，其中N=H*W
        
        # 通过MIL分类头，使用真实标签
        instance_logits, typicalities, soft_labels, cluster_labels = self.mil_head(x, labels)
        
        return instance_logits, typicalities, soft_labels, cluster_labels
...
if __name__ == "__main__":
    ...
    # 创建标签数据，形状为(B, H*W)，值为0或1
    y = torch.randint(0, 2, (1, 196)).to("cuda")  # 196 = 14*14 (224/16 = 14)
    
    # 运行模型
    instance_logits, typicalities, soft_labels, cluster_labels = medmamba_s(data, y)
    
    # 打印各个输出的形状
    print("Instance logits shape:", instance_logits.shape)
    print("Typicalities shape:", typicalities.shape)
